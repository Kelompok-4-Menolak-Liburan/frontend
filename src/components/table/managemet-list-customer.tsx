...
const ManagementListCustomer: React.FC<ManagementListCustomerProps> = ({
    ...
    name,
    email,
    ticketName, buyDate, price, isFree
}) => {
    return (
        <tr
            key={no}
        ...
        >
            ...
            {/* Ticket Name */}
            <td
                className="capitalize text-custom-green-normal font-semibold"
            >
                {ticketName}
            </td>
            {/* Price */}

            <td className="py-3">{isFree ? "Free" : (price !== undefined && getFormattedCurrency(price))}</td>
            {/* BUtton Edit */}
            <td className="py-3">
