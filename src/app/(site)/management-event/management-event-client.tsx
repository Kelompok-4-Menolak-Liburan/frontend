...
const ManagementEventClient: React.FC<ManagementEventClientProps> = ({
  data,
}) => {
  ...
  const sortOptions = ["Sold asc", "Sold desc", "View asc", "View desc"];
  ...
  const notActiveEvents = sortedData.filter(
  ...
  const tabs = [
    {
      label: "All",
      content: (
        <EventList events={sortedData} searchStatus={search.length > 0} />
      ),
    },
    {
      label: "Active",
      content: (
        <EventList events={activeEvents} searchStatus={search.length > 0} />
      ),
    },
    {
      label: "Not Active",
      content: (
        <EventList events={notActiveEvents} searchStatus={search.length > 0} />
      ),
    },
  ];
